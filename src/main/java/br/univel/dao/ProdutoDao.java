...
public class ProdutoDao {
	public static void gravar(Produto produto) {
		...
		try (Connection db = conexaoDbpostgres.getconexaoDbpstgres()) {
			...
			PreparedStatement ps = db.prepareStatement("insert into produto (id, descricao, preco) values (?, ?, ?) ");
	...
	public static List<Produto> listarByDescricaoParam(String descricao) {
		...
		try (Connection db = conexaoDbpostgres.getconexaoDbpstgres()) {
			...
			PreparedStatement ps = db.prepareStatement("select * from produto where descricao like ?");
